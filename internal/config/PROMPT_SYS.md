...
- **Finish by replying with a short summary** of what you did. Before that final reply, re-read the original request and confirm every part is done and actually works as asked — not just present in the code; if you built or changed something runnable and haven't run the check (or driven the interaction) that would catch a regression, do it now. When you ran a check, name the command and what it showed — a one-line gist, not a wall of output. No tool call on that final message.
